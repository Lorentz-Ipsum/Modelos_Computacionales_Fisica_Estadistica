...
public class OsciladoresApplet extends Applet implements ActionListener, ItemListener, Runnable {
   ...
   public synchronized void actionPerformed(ActionEvent var1) {
       if (var1.getSource() == this.calc) {
           ...
           for(int i = 0; i < this.BARRAS; ++i) {
               if (this.histo[i] > 1 && threshold == 0) {
                   threshold = i;
               }
           }

           GraficoAuxiliar var4 = new GraficoAuxiliar(this.anchura, this.altura, (double)threshold / (double)this.BARRAS, 1.0D, 0.0D, Math.floor(Math.log((double)this.maxHist)) + 1.0D, "S", "N(S)");
           var4.cuadro.FG_BG(false);
           var4.cuadro.setCol(Color.blue);

           for(int i = threshold; i < this.BARRAS; ++i) {
               var4.cuadro.CirculoRelleno((double)i / (double)this.BARRAS, Math.log((double)this.histo[i]), 4);
           }
   ...
   void inicializa() {
       ...
       double var9;
       for(i = 0; i < this.M; ++i) {
           var7 = var3 + (double)(this.radio - 10) * Math.cos((double)i * this.angulo_cajas);
           var9 = var5 - (double)(this.radio - 10) * Math.sin((double)i * this.angulo_cajas);
           double var11 = var3 + (double)(this.radio + 15) * Math.cos((double)i * this.angulo_cajas);
           double var13 = var5 - (double)(this.radio + 15) * Math.sin((double)i * this.angulo_cajas);
       ...
       if (var2 == 0) {
           ...
           for(i = 0; i < this.N; ++i) {
               var16 = this.fases[i] = Math.random() * 3.141592653589793D;
               var7 = var3 + (double)this.radio * Math.cos(var16);
               var9 = var5 - (double)this.radio * Math.sin(var16);
               this.principal.Recta(var3, var5, var7, var9);
               var20 = (int)Math.floor(var16 * (double)this.M / 2.0D / 3.141592653589793D);
       ...
       for(i = 0; i < this.M; ++i) {
           double var18 = (double)this.cuenta[i] / (double)this.N;
           if (this.cuenta[i] != 0) {
       ...
       if (var2 == 0) {
           for(i = 0; i < this.N; ++i) {
               this.frecuencias[i] = Math.random() * 2.0D * 3.141592653589793D;
           }
       } else if (var2 == 1) {
           for(i = 0; i < this.N; ++i) {
               this.frecuencias[i] = 0.5235987755982988D + (2.0D * Math.random() - 1.0D) * this.epsilon;
           }
       } else {
           for(i = 0; i < this.N; ++i) {
               this.frecuencias[i] = (double)i * 2.0D * 3.141592653589793D / (double)this.Nmax;
           }
       }
   ...
   void rota_fases() {
      ...
      for(i = 0; i < this.M; ++i) {
         double var19 = (double)this.cuenta[i] / (double)this.N;
         if (this.cuenta[i] != 0) {
      ...
      if (this.tiempo >= this.TMAX) {
         ...
         for(tframe = 0; tframe < this.TMAX / 2; ++tframe) {
            this.entropia.Recta((double)tframe, this.S[tframe], (double)(tframe + 1), this.S[tframe + 1]);
      ...
      tframe = (int)Math.floor(var17 * (double)this.BARRAS);
      var10002 = this.histo[tframe]++;
      if (this.histo[tframe] > this.maxHist) {
         this.maxHist = this.histo[tframe];
      }
