    ...
    if (running) {
        ...
        for (var step = 0; step < stepsPerFrame; step++) {
            ...
            if (step % (stepsPerFrame * 10) === 0) {
                document.getElementById(cvLectura).innerHTML = calcCv();
            }
        }
        for (var i = 0; i < sizeX; i++) {
            for (var j = 0; j < sizeY; j++) {
...
function calcCv() {
    ...
    e2Avg = e2Total / (sizeX * sizeY);
    aveE2 = Math.pow(eTotal / (sizeX * sizeY), 2);
